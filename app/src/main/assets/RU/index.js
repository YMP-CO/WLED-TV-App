...
function onLoad()
{
	...
	if (l.protocol == "file:") {
		...
		if (!locip) {
			locip = prompt("Enter the IP address of your WLED:");
	 if (locip === null) {
      alert("Enter the IP address of your WLED:")
      return
window.close()
        }
        	 if (locip === false) {
...
function generateListItemHtml(listName, id, name, clickAction, extraHtml = '', effectPar = '')
{
	return `<div class="lstI${id==0?' sticky':''}" data-id="${id}" ${effectPar===''?'':'data-opt="'+effectPar+'" '}onClick="${clickAction}(${id})">`+
		`<label title="(${id})" class="radio schkl" onclick="event.preventDefault()">`+ // (#1984)
			...
			`<span style="display: none;"  class="radiomark"></span>`+
			`<button class="lstIcontent">`+
				`<span class="lstIname">${name}</button>`+
			`</div>`+
		`</label>`+
		extraHtml +
	`</div>`;
...
function setEffectParameters(idx)
{
	...
	if (hasRGB && ((!controlDefined) || (paOnOff.length>0 && paOnOff[0]!="" && isNaN(paOnOff[0])))) {
		...
		else                                       pall.innerHTML = '<i class="icons sel-icon" onclick="tglHex()"></i> Цветные палитры:';
	} else {
		// disable palette list
		pall.innerHTML = '<i class="icons sel-icon" onclick="tglHex()"></i> ⚠️ Загрузка...';
