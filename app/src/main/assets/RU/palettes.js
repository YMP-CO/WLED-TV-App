...
async function onLoad()
{
	let l = window.location;
    	if (l.protocol == "file:") {
    		...
    		if (!locip) {
    			locip = prompt("Введите IP-адрес вашего WLED:");
    	 if (locip === null) {
          alert("Вы не ввели IP адрес!")
          return
    window.close()
            }
            	 if (locip === false) {
...
function populatePresets()
{
	...
	for (var key of (arr||[]))
	{
		...
		cn += `${isPlaylist(i)?"<i class='icons btn-icon'>&#xe139;</i>":""}<button class="lstIname">${pName(i)}</button></div>`;
    	pNum++;
	}
